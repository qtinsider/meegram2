...
QString Chat::type() const noexcept
{
    switch (m_typeId)
    {
        ...
        case td::td_api::chatTypeSupergroup::ID: {
            const auto *chatType = static_cast<const td::td_api::chatTypeSupergroup *>(m_chat->type_.get());
            return chatType && chatType->is_channel_ ? "channel" : "supergroup";
        }
        default:
            return {};
    }
