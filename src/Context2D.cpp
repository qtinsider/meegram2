...
QColor colorFromString(const QString &name)
{
    ...
    else if (name.startsWith("rgb("))
    {
        ...
        return QColor((int)qClamp(compo[0], qreal(0), qreal(255)), (int)qClamp(compo[1], qreal(0), qreal(255)), (int)qClamp(compo[2], qreal(0), qreal(255)));
    }
    else if (name.startsWith("hsla("))
    {
        ...
        return QColor::fromHslF(compo[0], compo[1], compo[2], compo[3]);
    }
    else if (name.startsWith("hsl("))
    {
        ...
        return QColor::fromHslF(compo[0], compo[1], compo[2]);
    }
    else
    {
        //QRgb color;
...
void Context2D::transform(qreal m11, qreal m12, qreal m21, qreal m22, qreal dx, qreal dy)
{
...
void Context2D::setTransform(qreal m11, qreal m12, qreal m21, qreal m22, qreal dx, qreal dy)
{
    QMatrix mat(m11, m12, m21, m22, dx, dy);
    m_state.matrix = mat;
    m_state.flags |= DirtyTransformationMatrix;
...
CanvasGradient *Context2D::createLinearGradient(qreal x0, qreal y0, qreal x1, qreal y1)
{
    QLinearGradient g(x0, y0, x1, y1);
    return new CanvasGradient(g);
}

CanvasGradient *Context2D::createRadialGradient(qreal x0, qreal y0, qreal r0, qreal x1, qreal y1, qreal r1)
{
...
{
    if (m_shadowbuffer.isNull() || m_shadowbuffer.width() != m_width + m_state.shadowOffsetX || m_shadowbuffer.height() != m_height + m_state.shadowOffsetY)
    {
...
void Context2D::bezierCurveTo(qreal cp1x, qreal cp1y, qreal cp2x, qreal cp2y, qreal x, qreal y)
{
...
void Context2D::arcTo(qreal x1, qreal y1, qreal x2, qreal y2, qreal radius)
{
    ...
    m_path.arcTo(st.x(), st.y(), end.x() - st.x(), end.y() - st.y(), radius, 90);
}
...
void Context2D::arc(qreal xc, qreal yc, qreal radius, qreal sar, qreal ear, bool anticlockwise)
{
    //### HACK
...
void Context2D::beginPainting()
{
    ...
    if (m_state.shadowBlur > 0 && m_painter.device() != &m_shadowbuffer)
    {
        ...
        m_painter.setViewport(m_state.shadowOffsetX, m_state.shadowOffsetY, m_shadowbuffer.width(), m_shadowbuffer.height());
        m_shadowbuffer.fill(Qt::transparent);
    }
...
void Context2D::endPainting()
{
    if (m_state.shadowBlur > 0)
    {
        ...
        if (m_shadowColorIndexBuffer.isEmpty() || m_shadowColorBuffer != m_state.shadowColor)
        {
            ...
            for (int i = 0; i < 256; ++i)
            {
                m_shadowColorIndexBuffer << qRgba(qRound(255 * m_state.shadowColor.redF()), qRound(255 * m_state.shadowColor.greenF()),
                                                  qRound(255 * m_state.shadowColor.blueF()), i);
            }
        }
...
void Context2D::drawImage(const QVariant &var, qreal sx, qreal sy, qreal sw = 0, qreal sh = 0)
{
