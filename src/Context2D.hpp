...
class CanvasGradient : public QObject
{
    Q_OBJECT
public:
    CanvasGradient(const QGradient &gradient)
        : m_gradient(gradient)
    {
    }
...
public slots:
    ...
    void addColorStop(float pos, const QString &color)
    {
        m_gradient.setColorAt(pos, colorFromString(color));
    }
...
public:
    ...
    CanvasImage(const QString &url)
        : m_image(url)
        , m_src(url)
    {
    }
    CanvasImage(const QPixmap &pixmap)
    {
        m_image = pixmap;
    }
...
public slots:
    void save();     // push state on state pplStack
    void restore();  // pop state pplStack and restore state

    void fillText(const QString &text, qreal x, qreal y);
    void strokeText(const QString &text, qreal x, qreal y);

    void setInPaint(bool val)
    {
        m_inPaint = val;
    }
...
private:
    ...
    enum DirtyFlag {
        ...
        MDirtyPen = DirtyStrokeStyle | DirtyLineWidth | DirtyLineCap | DirtyLineJoin | DirtyMiterLimit,
