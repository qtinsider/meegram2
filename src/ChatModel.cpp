...
QVariant ChatModel::data(const QModelIndex &index, int role) const
{
    ...
    switch (role)
    {
        ...
        case IsPinnedRole:
            return getChatPosition(chatPtr, m_list)->isPinned();
...
void ChatModel::sortChats()
{
    ...
    std::ranges::sort(m_chats, std::greater{}, [&](auto &&chatPtr) {
        ...
        {
            if (auto pos = getChatPosition(chat, m_list))
            {
                return pos->order();
            }
        }
...
ChatPosition *ChatModel::getChatPosition(const std::shared_ptr<Chat> &chat, const std::unique_ptr<ChatList> &list) const
{
    const auto &positions = chat->positions();

    if (auto it = std::ranges::find_if(positions,
                                       [&](const auto &position) {
                                           auto positionList = position->list();
                                           return positionList->type() == list->type() && positionList->folderId() == list->folderId();
                                       });
        it != positions.end())
    {
        return it->get();
