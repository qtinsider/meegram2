...
QVariant ChatModel::data(const QModelIndex &index, int role) const
{
    ...
    switch (role)
    {
        ...
        case LastMessageSenderRole:
            return "Chikodili" /*Utils::getMessageSenderName(chat->last_message_, m_storageManager, m_locale)*/;
        case LastMessageContentRole:
            return "Hi there!" /*Utils::getContent(chat->last_message_, m_storageManager, m_locale)*/;
        case LastMessageDateRole: {
            return "12:45" /*Utils::getMessageDate(chat->last_message_, m_locale)*/;
        }
        case IsPinnedRole:
...
struct Sorter
{
    Sorter(StorageManager *store, ChatList chatList)
        : store_(store)
        , chatList_(std::move(chatList))
    {
    }

    bool operator()(auto a, auto b) const
    {
        const auto chatA = store_->getChat(a);
        const auto chatB = store_->getChat(b);
        ...
        auto orderA = Utils::getChatPosition(chatA, chatList_)->order_;
        auto orderB = Utils::getChatPosition(chatB, chatList_)->order_;
